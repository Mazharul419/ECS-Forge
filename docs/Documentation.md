...
!!! success "Pattern demonstrated"

    Provider-level Resource tagging - this tagging enables clear ownership of resources based on project, environments, tech stack and the github repo associated with this - this in turn betters FinOps by showing clear breakdown of costs and helps identify what resources are associated with this project.
...
=== "Explanation"
    ...
    Internet Protocol Version 4 (IPv4) was used to define this block
    ...
    >
    >IPv4 is a set of communication rules that [provides data exchange over the internet](https://aws.amazon.com/compare/the-difference-between-ipv4-and-ipv6/)

    !!! warning "Improvement needed"
    
        Given the expansion of devices and limited addressing offered by this protocol and the improved features such as [autoconfiguration, improved routing and security,](https://aws.amazon.com/compare/the-difference-between-ipv4-and-ipv6/) it is worth considering IPv6 or dual-stack (IPv6 with backwards IPv4 compatibility) for the next project!
    ...
    !!! success "Pattern demonstrated"
        ...
        Resource-level tagging - this identifies resources when looking at cost explorer - it also had the excellent effect of helping me leftover resources when setting up destroy workflows in Terragrunt and CD.
    ...
    !!! success "Pattern demonstrated"
        ...
        2 AZs for public subnets - high availability. This is designing for fault isolation too - both of which fall under the [Reliability pillar of the AWS Well-Architected Framework](https://docs.aws.amazon.com/wellarchitected/latest/reliability-pillar/rel_fault_isolation_multiaz_region_system.html).
    ...
    !!! information "Information"
        ...
        This argument pulls multiple AZs, however for the resource as a whole it is limited to 2 due to the earlier variabilised `count` argument, where both environments have two hardcoded subnet CIDRs
...
=== "Terraform module"
    ``` title="infrastructure/modules/security-groups/main.tf"
    --8<-- "infrastructure/modules/security-groups/main.tf"
    ```

=== "Terragrunt configs"

    ``` title="dev/security-groups/terragrunt.hcl"
    --8<-- "infrastructure/live/dev/security-groups/terragrunt.hcl"
    ```
    ``` title="dev/env.hcl"
    --8<-- "infrastructure/live/dev/env.hcl"
    ```
    ``` title="Prod"
    --8<-- "infrastructure/live/prod/security-groups/terragrunt.hcl"
    ```
    ``` title="prod/env.hcl"
    --8<-- "infrastructure/live/prod/env.hcl"
    ```
...
=== "Terraform module"
    ``` title="infrastructure/modules/vpc-endpoints/main.tf"
    --8<-- "infrastructure/modules/vpc-endpoints/main.tf"
    ```

=== "Terragrunt configs"

    ``` title="dev/vpc-endpoints/terragrunt.hcl"
    --8<-- "infrastructure/live/dev/vpc-endpoints/terragrunt.hcl"
    ```
    ``` title="dev/env.hcl"
    --8<-- "infrastructure/live/dev/env.hcl"
    ```
    ``` title="Prod"
    --8<-- "infrastructure/live/prod/vpc-endpoints/terragrunt.hcl"
    ```
    ``` title="prod/env.hcl"
    --8<-- "infrastructure/live/prod/env.hcl"
    ```
...
Environments are configured with specific values at the environment level (dev/prod) or the project level.
