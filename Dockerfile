...
RUN npm install
RUN npm run build
RUN VERSION=4.112.0 npm run build:vscode --mount=type=secret,id=GITHUB_TOKEN, env=GITHUB_TOKEN
...
RUN useradd -m coder
COPY --from=builder --chown=coder:coder /usr/src/app/release-standalone/ ./
